...
// ... (renderHtml and other helper functions remain same)
function escapeHtml(v) { return String(v || '').replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":"&#39;"}[m])); }
function buildTemplateBackground(tp) {
  if (!fs.existsSync(tp)) return 'background-color: #f8f9fa;';
  return `background-image: url('data:image/png;base64,${fs.readFileSync(tp).toString('base64')}'); background-size: cover; background-position: center;`;
}
function getDefaultPhotoBase64() {
  const p = path.resolve(__dirname, '../../../frontend/src/assets/dove_ia.png');
  return fs.existsSync(p) ? `data:image/png;base64,${fs.readFileSync(p).toString('base64')}` : null;
}
function getFontUrl(f) { return `file://${path.join(__dirname, '../assets/fonts', f).replace(/\\/g, '/')}`; }
